...
export default function SearchResults({ bibliographie }) {
  const bibliographie_list =
    bibliographie.all_bibliographie?.data.map((el, i) => {
      return (
        <li key={`${el}-${i}`} className="hit-item">
          <a href={el.lien_source} target="_blank" rel="noreferrer">
  ...
  return (
    <Template classNamePage="bibliographie" pageName="Publications">
      ...
      <div className="bibliographie-list-wrapper">
        <ul className="bibliographie-list">{bibliographie_list}</ul>
      </div>
    </Template>
  )
}
