...
export default function Timeline({
  ...
  let years = []
  datas.map((e) => {
    !years.includes(e.debut.substr(0, 4)) &&
      e.debut.substr(0, 4) >= 2010 &&
      years.push(e.debut.substr(0, 4))
    !years.includes(e.fin !== null && e.fin.substr(0, 4)) &&
      e.fin !== null &&
      e.fin.substr(0, 4) >= 2010 &&
      years.push(e.fin.substr(0, 4))
  })
  ...
  const themeDescription = () => {
    return {
      __html: enquetes.all_themes.data.find(
        (e) => e.id.toString() === enquetes_id,
      ).description,
    }
  }
  ...
  const Hit = ({ hit }) => {
    let date = (
      <div
        className={`hit-item__date${
          hit.fin !== null && hit.fin !== hit.debut ? " period" : ""
        }`}
      >
        {!searchResults ? (
          <>
            <p>{dateFormatter(hit.debut)}</p>
            {hit.fin !== null && hit.fin !== hit.debut && (
              <p>
                <span>-</span>
                <br />
                {dateFormatter(hit.fin)}
              </p>
            )}
          </>
        ) : (
    ...
    let image =
      hit.image !== null &&
      images.files.data.find((image) => hit.image.id === image.id)
    return (
      <div
        id={hit.debut.substr(0, 4)}
        className={`${searchResults ? "results-item" : "timeline-item"}`}
        ...
        onClick={() => {
          setDrawer({
            ...
            bibliographie:
              hit.bibliographie &&
              hit.bibliographie.find((e) => e) !== undefined &&
              hit.bibliographie.find((e) => e).evenements_id,
          })
        }}
      >
        <span
          className="peak"
          style={{ "--beforeHeightItem": beforeHeightItem }}
        ></span>
        <div
          className={`hit-item__content ${
            image && image.data ? "" : "fullwidth"
          }`}
        >
          {!searchResults && date}
          <h3 className="hit-item__title">
            {hit.titre.length > (image && image.data ? 100 : 125)
              ? extract(hit.titre, image && image.data ? 100 : 125)
              : hit.titre}
              ...
              {hit.categorie.map((category, i) => {
                if (category !== "") {
                  ...
                  return (
                    <div
                      key={category + "-" + i}
                      className="hit-item__category"
                    >
                      <span className="hit-item__category__tooltiptext">
                        {cleanCategories[category]}
                      </span>
                      <span
                        className="hit-item__category__tooltip"
                        style={{ "--category": customColor }}
                      />
                    </div>
                  )
                }
        ...
        {!searchResults && image && image.data ? (
          <div
            className="hit-item__image"
            style={{
              backgroundImage: `url(${
                image ? image.data.thumbnails[3].url : ""
              })`,
            }}
            alt=""
          />
            ...
            return (
              <li
                className={`refinement-list-item ${
                  item.isRefined ? "refinement-list-item--selected" : ""
                }`}
                key={item.label}
              >
                <a
                ...
                >
                  <div className="refinement-list-labelText">
                    <span
                      className="dot"
                      style={{ "--category": switchColors(item.label) }}
                    />
  ...
  return (
    <InstantSearch indexName="resistic" searchClient={searchClient}>
      {enquetes_name && (
        <RefinementList
      ...
      {searchResults && (
        <div>
          <SearchBox
            reset={<i className="icon-close"></i>}
            autoFocus
      ...
      {!searchResults && (
        <div className="heading">
          <div className="filters">
            <CustomRefinementList attribute="categorie" />
          </div>
        </div>
      )}
      ...
      <div className={`timeline-wrapper ${enquetes_name ? "" : "fullwidth"}`}>
        {enquetes_name && (
          <div className="timeline-themes-content">
            <div className="timeline-themes-content-wrapper">
              <p className="upper-heading">Enquêtes</p>
              <h1 style={{ marginBottom: "1em" }}>
                {
                  enquetes.all_themes.data.find(
                    (e) => e.id.toString() === enquetes_id,
                  ).theme
                }
              </h1>
              <div
                className="timeline-themes-description"
                dangerouslySetInnerHTML={themeDescription()}
              />
            </div>
          </div>
        )}
        {!enquetes_name && !searchResults && (
          <ul className="timeline-years">
            {years
              ...
              .map((e) => {
                return (
                  <li key={e}>
                    <a href={`#${e}`}>{e === "2010" ? "Avant 2010" : e}</a>
                  </li>
                )
              })}
          </ul>
